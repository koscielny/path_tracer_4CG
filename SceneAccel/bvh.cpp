...
bool BVH::getIntersection(Ray& ray, Intersection* intersection, bool occlusion) const {
    /* Initialize intersection. */
	intersection->travel = std::numeric_limits<float>::infinity();
	intersection->primative = nullptr;
 float bbhits[4];
 ...
 todo[stackptr].i = 0;
 todo[stackptr].mint = -std::numeric_limits<float>::infinity();
 ...
 while(stackptr>=0) {
  ...
  if (near > intersection->travel)
   continue;

  // Is leaf -> Intersect
  if( node.rightOffset == 0 ) {
   for(uint32_t o=0;o<node.nPrims;++o) {
	   Shape* primitive = (*build_prims)[node.start + o];
	   double distance = primitive->intersect(ray);
	   if ((distance >= 0) && (distance < intersection->travel))
                {
					intersection->primative = primitive;
					intersection->travel = distance;
					intersection->sourceDirection = -ray.direction;
					intersection->point = ray.getPoint(distance);
                    /* If we just want occlusion, any intersection is sufficient. */
 ...
 return intersection->primative != NULL;
}
...
: leafSize(leafSize), build_prims(objects), flatTree(NULL), nNodes(0), nLeafs(0) {
...
void BVH::build()
{
 ...
 while(stackptr > 0) {
		...
		AABB2 bb( (*build_prims)[start]->BoundingBox());
		AABB2 bc( (*build_prims)[start]->Centroid());
		for(uint32_t p = start+1; p < end; ++p) {
			bb.expandToInclude( (*build_prims)[p]->BoundingBox());
			bc.expandToInclude( (*build_prims)[p]->Centroid());
		}
		node.bbox = bb;
  ...
  // size, then this will become a leaf. (Signified by rightOffset == 0)
		...
		for(uint32_t i=start;i<end;++i) {
			if( (*build_prims)[i]->Centroid()[split_dim] < split_coord ) {
